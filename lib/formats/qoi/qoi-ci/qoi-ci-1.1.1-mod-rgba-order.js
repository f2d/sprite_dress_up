...
QOIEncoder.canEncode = function(width, height, alpha)
{
	return (
		width > 0
	&&	height > 0
	&&	height <= ((2147483625 / width | 0) / (alpha ? 5 : 4) | 0)
	);
}
...
QOIEncoder.prototype.encode = function(width, height, pixels, alpha, linearColorspace)
{
	...
	const pixelsSize = width * height;
	const index = new Int32Array(64);
	const encoded = new Uint8Array(14 + pixelsSize * (alpha ? 5 : 4) + 8);
	...
	for (let pixelsOffset = 0; pixelsOffset < pixelsSize;) {
		const pixel = pixels[pixelsOffset++];
...
QOIEncoder.prototype.encodeFasterIndexButTwiceRGBA = function(width, height, pixels, alpha, linearColorspace)
{
	if (pixels == null || !QOIEncoder.canEncode(width, height, alpha))
		return false;
...
QOIDecoder.prototype.decode = function(encoded, encodedSize)
{
	...
	const pixelsSize = width * height;
	const pixels = new Int32Array(pixelsSize);
	const index = new Int32Array(64);
...
*/
		...
		case 2:
			e -= 160;
			const rb = encoded[encodedOffset++];
//* RGBA:
			pixel = (
				(pixel & -16777216)
			|	((pixel + ((e + (rb >> 4) - 8)      )) & 255)
