...
//* Some programs (e.g. MyPaint) reject ORA files where not every layer has an image, so we add an 1px transparent PNG:
		...
		function onImagesComplete() {
			...
			const	imagesToSaveByPrefix = {};
				...
				const	imageList = imagesByPrefix[srcNamePrefix];
...
//* Add image files and paths, batched by prefix:
...
//* Create and return ZIP file content:
...
//* Keeping track of finished loading tasks:
		...
		function onStepDone(imageEntry, content) {
			++stepsDone;

			if (imageEntry && content) {
				imageEntry.content = content;
			}
