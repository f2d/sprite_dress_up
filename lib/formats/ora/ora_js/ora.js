		...
		var	element = parentElement.firstChild;
			...
			var	child = null;
				...
				var	mask = (
						...
						child.mask_fill_outside
						? {}
						: null
					);

					if (mask) {
						for (var key in child) if (hasPrefix(key, MASK_ATTRIBUTE_PREFIX)) {
							mask[key.slice(MASK_ATTRIBUTE_PREFIX.length)] = child[key];
						}

						loadSrcImage(child.mask = mask);
					}
				}
	...
	OraFile.prototype.save = function (onSaveComplete, onError, onProgressByKey) {
		...
		function addLayers(parentElement, layers) {
			...
			const	mask = node.mask;
				...
				if (mask) {
					for (var key in mask) {
						node[MASK_ATTRIBUTE_PREFIX + key] = mask[key];
					}
				}
