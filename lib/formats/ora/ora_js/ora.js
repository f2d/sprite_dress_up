...
//* Draw layer onto a new canvas element:
	...
	Layer.prototype.toCanvas = function (canvas, width, height, ignoreOffset) {
		return imageToCanvasFromLayer(this, canvas, width, height, ignoreOffset);
	};

	Layer.prototype.maskToCanvas =
	Stack.prototype.maskToCanvas = function (canvas, width, height, ignoreOffset) {
		return imageToCanvasFromLayer(this.mask, canvas, width, height, ignoreOffset);
	};
...
//* Old Firefox (e.g. v56) throws an empty exception at importStylesheet; without a try-catch it just silently stops.
	...
	function imageToCanvasFromLayer(imageHolder, canvas, width, height, ignoreOffset) {
	...
	const	image = imageHolder.image;
		...
		,	ignoreOffset ? 0 : imageHolder.x
		,	ignoreOffset ? 0 : imageHolder.y
		);
	}
...
var	emptyImageDataURL;
	...
	function getEmptyImageDataURL() {
		return emptyImageDataURL || (emptyImageDataURL = imageToCanvas(null, null, 1,1).toDataURL('image/png'));
	}
