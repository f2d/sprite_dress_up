	...
	OraFile.prototype.load = function (blob, onLoadComplete, onError, onProgress) {
		...
		const	srcPath = imageHolder.src;
			...
			if (global.ora.preloadImages) {
				extractImage(srcPath, fs, onImageLoad, onStepDone, onStepDone);
			} else {
...
//* Saving to a file:
	...
	OraFile.prototype.saveOld = function (onSaveComplete, onError, onZipProgress) {
		this.fs.exportBlob(onSaveComplete, onZipProgress, onError);
	};

	OraFile.prototype.save = function (onSaveComplete, onError, onZipProgress, onImagesProgress) {
		...
		function addLayerImage(element, content, attrNamesDict, attrNameKey, srcBaseName) {
		const	fileName = srcBaseName + '.png';
		const	attrName = getAttributeNameFromDict(attrNamesDict, attrNameKey);
		const	srcPath = LAYER_IMAGE_FOLDER + '/' + fileName;
			element.setAttribute(attrName, srcPath);
...
//* Some programs (e.g. MyPaint) reject ORA files where not every layer has an image, so we add an 1px transparent PNG:
			...
			if (!content) {
				content = imageToCanvas(null, null, 1,1);
			}
...
//* Do not reencode via canvas if not needed:
			...
			&&	hasPrefix(content, 'blob:')
				...
				getBlobFromUrl(content, function (evt) {
					onStepDone(fileName, evt.target.response);
				}, onError);
			} else {
				onStepDone(fileName, content);
			}
		}
		...
		const	fileNames = [];
			...
			for (const fileName in imagesByFileName) {
				fileNames.push(fileName);
			}

			arraySortNumeric(fileNames).map(
				function (fileName) {
					addFileToArchiveNode(fsDataNode, fileName, imagesByFileName[fileName]);
				}
			);
...
//* Always compress XML text file:
					...
					fs.exportBlob(onSaveComplete, onZipProgress, onError);
				}
			];
...
//* Keeping track of finished loading tasks:
		...
		function onStepDone(fileName, content) {
			++stepsDone;

			if (fileName && content) {
				imagesByFileName[fileName] = content;
			}

			if (onImagesProgress) {
				onImagesProgress(stepsDone, stepsTotal);
			}
...
//* Optionally compress PNGs:
		...
		if (typeof fs.compressionLevelDefault !== 'undefined') {
			fs.compressionLevelDefault = (global.ora.zipCompressImageFiles ? 6 : 0);
		}
...
//* Source: https://stackoverflow.com/a/17772086
	...
	function arraySortNumericCompare(a,b) { return a.localeCompare(b, 'en', {numeric: true}); }
	function arraySortNumeric(array) {
		if (
			typeof global.Intl === 'function'
		||	typeof global.Intl === 'object'
		) {
			array.sort(new Intl.Collator('en', {numeric: true}).compare);
		} else
		if (typeof global.String.prototype.localeCompare === 'function') {
			array.sort(arraySortNumericCompare);
		} else {
			array.sort();
		}
