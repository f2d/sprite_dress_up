...
const	ROOT_IMAGE_ATTRIBUTE_NAMES_BY_KEY = {
		width  : ['w', 'width',  'image-width']
	,	height : ['h', 'height', 'image-height']
	,	version : ['version', 'v', 'ora-version']
	};
...
//* Custom-added properties, not in spec version 0.0.5:
	...
	,	width  : ['w', 'width',  'image-width']
	,	height : ['h', 'height', 'image-height']
	,	image_file_type : ['type', 'file-type', 'image-type', 'mime-type', 'content-type']
	...
	,	mask_x : ['mask-x', 'mask-left', 'mask-image-left', 'mask-image-x']
	,	mask_y : ['mask-y', 'mask-top',  'mask-image-top',  'mask-image-y']
	,	mask_width  : ['mask-w', 'mask-width',  'mask-image-width']
	,	mask_height : ['mask-h', 'mask-height', 'mask-image-height']
	,	mask_image_file_type : ['mask-type', 'mask-file-type', 'mask-image-type', 'mask-mime-type', 'mask-content-type']
	};
...
//* Add layers to document tree:
		...
		if (rootLayers.length > 0) {
			if (
				rootLayers.length === 1
			&&	isTrivialStack(rootLayers[0])
			) {
				addLayers(imageElement, rootLayers);
			} else {
			const	rootStack = new Stack(ROOT_STACK_DEFAULT_NAME);
...
//* Note: dataURI has lower length limits than blobs.
	...
	function extractImage(path, zipFile, fileType, onImageLoad, onDone, onError) {
		...
		const	image = new global.Image();
			...
			image.file_mod_time = getModTime(file.data || file);
