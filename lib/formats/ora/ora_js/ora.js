...
//* ORA file parser, spec version 0.05 + custom extensions.
...
//* http://www.freedesktop.org/wiki/Specifications/OpenRaster/Draft/
	...
	OraFile.prototype.load = function (blob, onComplete, onError, onProgress) {
		...
		function loadLayers(layeredImage, onCount, onDone) {
			...
			function setAttributesFromElement(layerOrStack, element, attributeNamesByKey) {
			...
			const	attributes = element.attributes;
				...
				const	key = getPropertyKeyByAttributeName(name);
					...
					if (key) {
						...
						layerOrStack.otherAttributes[name] = value;
					}
				}
			}

			function loadChildLayer(layerElement) {
			const	layer = new Layer();

				setAttributesFromElement(layer, layerElement, LAYER_ATTRIBUTE_NAMES_BY_KEY);
				loadSrcImage(layer, onCount, onDone);

				return layer;
			}

			function loadChildStack(stackElement) {
		...
		const	layersCount = oraFile.layersCount = layeredImage.getElementsByTagName('layer').length;
		const	stacksCount = oraFile.stacksCount = layeredImage.getElementsByTagName('stack').length;
...
//* TODO: save subfolders and layer masks
	...
	function addNestedNodeToArchive(node, path) {
		...
		if (path.map) {
			path.map(
				function (subName) {
					node = node.addDirectory(subName || 'unnamed_folder');
				}
			);
		}
	...
	const	fileName = path.pop() || 'unnamed_file';
...
//* Note: onLoad = callback function with the loaded object as parameter.
	...
	function loadFile(blob, onComplete, onError, onProgress) {
	const	oraFile = new OraFile();

		oraFile.load(blob, onComplete, onError, onProgress);
	}
