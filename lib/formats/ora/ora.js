...
//* These are custom-added, not in Open Raster Spec version 0.05:

var	optionalAttriubutes = [
		'mask_x'
	,	'mask_y'
	,	'mask_src'
	,	'clipping'
...
var	optionalAttriubuteValueToSkip = [
		'0'
	,	'no'
	,	'none'
	,	'null'
	...
	var	that = this;
		...
		var	loadImage = function(src, onDone, onError) {
				if (src) {
					...
					extractImage(src, fs, onDone, onError);
				}
			};
...
//* Keeping track of finished loading tasks:
	...
	var	fsDataDir    = 'data';
	var	fsDataNode   = fs.root.addDirectory(fsDataDir);
	var	fsThumbsNode = fs.root.addDirectory('Thumbnails');
...
//* TODO: save subfolders and layer masks
	...
	var	layerCount = this.layers.length;
		...
		var	layerElem = xmlDoc.createElement('layer');
			layerElem.setAttribute('src'         , fsDataDir + '/' + fileName);
		...
		var	url = tmpCanvas.toDataURL('image/png');
			...
			var	url = tmpCanvas.toDataURL('image/png');
				fsThumbsNode.addData64URI('thumbnail.png', url);
...
//* Paint result:
	...
	var	imgData = ctx.getImageData(0,0, oraFile.width, oraFile.height),
		...
		while (layersCount > layerIdx) {
			...
			if (layer && layer.image && (layer.visibility === 'visible' || typeof layer.visibility === 'undefined')) {
