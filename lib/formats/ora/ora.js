	...
	var	that = this;
		...
		var	loadImage = function(src, onDone, onError) {
				if (src) {
					...
					extractImage(src, fs, onDone, onError, enableImageAsBlob);
				}
			};
...
//* Note: onload = callback with the loaded object as parameter.
	...
	function setZipParams() {
		zip.workerScriptsPath = obj.ora.scriptsPath || zip.workerScriptsPath || '';
		zip.useWebWorkers = !!(
			typeof obj.ora.enableWorkers !== 'undefined'
			? obj.ora.enableWorkers
			: zip.useWebWorkers
		);
	}
