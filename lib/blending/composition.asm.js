...
//* Declare functions, shared inside module: ----------------------------------
	...
	function init_vars(data_length) {
		...
		valueToAlphaDivisor = +(MAX_VALUE|0);
	...
	function blend_alpha(i,j) {
		...
		if ((valueAbove|0) ==  (MAX_VALUE|0)) alphaAbove =  MAX_ALPHA;
			...
			a = +(valueAbove|0);
			alphaAbove = +(a / valueToAlphaDivisor);
		}
		...
		// else {
			// b = +(valueBelow|0);
			// alphaBelow = +(b / valueToAlphaDivisor);
		// }
		...
		if ((valueBelow|0) ==  (MAX_VALUE|0)) valueResult = MAX_VALUE|0;
			...
			b = +(valueBelow|0); //* int to double

			valueResult = ~~floor(b + (a * alphaAbove)); //* double to int
		}
		...
		// a = +(valueResult|0);
		// alphaDiff  = +(a - alphaBelow);
		// alphaMixed = +(a / valueToAlphaDivisor);
...
//* RGBexpression: C = B + A - 1;
		...
		a = +((valueAbove - MAX_VALUE)|0);
		b = +(valueBelow|0);

		valueResult = ~~floor(b + (a * alphaAbove));
