...
//* https://stackoverflow.com/a/6386389
	...
	function blend_alpha(channel) {
		...
		return float_to_int_in_clamped_range((MAX_NORMALIZED_FLOAT - (inverse_a * inverse_b)) * CLAMPED_TO_NORMALIZED_RATIO)|0;
	}
	...
	function run_cycle_substract(data_length) {
		...
		do {
			...
			if (uint8[indexAbove|3]|0) {
				if (uint8[indexBelow|3]|0) {
					if (intResultA = blend_alpha(3)|0) {
						intResultB = blend_color_substract(2)|0;
						intResultG = blend_color_substract(1)|0;
						intResultR = blend_color_substract(0)|0;
						save_result();
					} else uint32[indexBelow >> 2] = 0;
				} else uint32[indexBelow >> 2] = uint32[indexAbove >> 2];
			}
