...
//* Declare constants, shared inside module: ----------------------------------
	...
	var MIN_CLAMPED_INT = 0;
	var MAX_CLAMPED_INT = 255;

	var ALMOST_MIN_CLAMPED_INT = 1;
	var ALMOST_MAX_CLAMPED_INT = 254;

	var MIN_CLAMPED_FLOAT = 0.0;
	var MAX_CLAMPED_FLOAT = 255.0;

	var MIN_NORMALIZED_FLOAT = 0.0;
	var MAX_NORMALIZED_FLOAT = 1.0;

	var CLAMPED_TO_NORMALIZED_RATIO = 255.0;
...
//* and clamping at this stage ruins everything.
		...
		// if ((i|0) == (MIN_CLAMPED_INT|0)) return +MIN_CLAMPED_FLOAT;
		// if ((i|0) == (MAX_CLAMPED_INT|0)) return +MAX_CLAMPED_FLOAT;
	...
	function int_to_float_in_normalized_range(i) {
		...
		// if ((i|0) == (MIN_CLAMPED_INT|0)) return +MIN_NORMALIZED_FLOAT;
		// if ((i|0) == (MAX_CLAMPED_INT|0)) return +MAX_NORMALIZED_FLOAT;
	...
	function int_in_clamped_range(i) {
		...
		if ((i|0) <= (MIN_CLAMPED_INT|0)) return MIN_CLAMPED_INT|0;
		if ((i|0) >= (MAX_CLAMPED_INT|0)) return MAX_CLAMPED_INT|0;
	...
	function float_to_int_in_clamped_range(f) {
		...
		if (f <= MIN_CLAMPED_FLOAT) return MIN_CLAMPED_INT|0;
		if (f >= MAX_CLAMPED_FLOAT) return MAX_CLAMPED_INT|0;
	...
	function blend_alpha(channel) {
		...
		if ((valueAbove|0) == (MIN_CLAMPED_INT|0)) return valueBelow|0;
		if ((valueAbove|0) == (MAX_CLAMPED_INT|0)) return MAX_CLAMPED_INT|0;
		if ((valueBelow|0) == (MAX_CLAMPED_INT|0)) return MAX_CLAMPED_INT|0;
...
//* C = B / A; (?)
	...
	function blend_color_divide(channel) {
		...
		if ((valueAbove|0) == (MIN_CLAMPED_INT|0)) valueAbove = ALMOST_MIN_CLAMPED_INT;	//* <- avoid division by zero
...
//* A+B > 1: C = 1;
	...
	function blend_color_hard_mix(channel) {
		...
		if (((valueAbove + valueBelow)|0) > (MAX_CLAMPED_INT|0)) {
			opaqueResult = MAX_CLAMPED_INT|0;
		}
...
//* A >  0.5: C = B / (2*(1-A))
	...
	function blend_color_vivid_light(channel) {
		...
		if ((valueAbove|0) < 128) {
			...
			if ((valueAbove|0) == (MAX_CLAMPED_INT|0)) valueAbove = ALMOST_MAX_CLAMPED_INT;	//* <- avoid division by zero
...
//* transit from B to A to the depth of T mask:
	...
	function blend_transition(channel) {
		...
		if ((valueTMask|0) == (MIN_CLAMPED_INT|0)) return valueBelow|0;
		if ((valueTMask|0) == (MAX_CLAMPED_INT|0)) return valueAbove|0;
		...
		return float_to_int_in_clamped_range(((a * m) + (b * n)) / MAX_CLAMPED_FLOAT)|0;
	}
...
//* Declare public functions: -------------------------------------------------
	...
	function run_cycle_transition(data_length) {
		...
		do {
			...
			valueTFrom = (MAX_CLAMPED_INT - valueTMask)|0;
