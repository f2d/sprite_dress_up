...
//* 3) Transit to color above to degree of transparency below.
	...
	function float_opaque_to_transparent_color(float_after) {
		float_after = +float_after;

		var float_below = 0.0;
		var float_above = 0.0;

		if (normalizedAlphaAbove < MAX_NORMALIZED_FLOAT) {
			float_below = +(intValueBelow|0);

			if (normalizedAlphaAbove > MIN_NORMALIZED_FLOAT) {
				float_after = +(
					(float_after * normalizedAlphaAbove)
				+	(float_below * (MAX_NORMALIZED_FLOAT - normalizedAlphaAbove))
				);
			} else {
				float_after = +float_below;
			}
		}

		if (normalizedAlphaBelow < MAX_NORMALIZED_FLOAT) {
			float_above = +(intValueAbove|0);

			if (normalizedAlphaBelow > MIN_NORMALIZED_FLOAT) {
				float_after = +(
					(float_after * normalizedAlphaBelow)
				+	(float_above * (MAX_NORMALIZED_FLOAT - normalizedAlphaBelow))
				);
			} else {
				float_after = +float_above;
			}
		}
	...
	function int_opaque_to_transparent_color(int_after) {
		int_after = int_after|0;

		return float_opaque_to_transparent_color(+(int_after|0))|0;
	}
...
//* C = B + A;
	...
	function blend_color_add(channel) {
		...
		return int_opaque_to_transparent_color(int_after)|0;
	}
...
//* C = B / A;
	...
	function blend_color_divide(channel) {
		...
		return float_opaque_to_transparent_color(float_after * CLAMPED_TO_NORMALIZED_RATIO)|0;
	}
...
//* C = min(max(2*A - 1, B), 2*A)
	...
	function blend_color_pin_light(channel) {
		...
		if ((intValueBelow|0) < (int_b|0)) return int_opaque_to_transparent_color(int_b)|0;
		if ((intValueBelow|0) > (int_a|0)) return int_opaque_to_transparent_color(int_a)|0;
...
//* transit from B to A to the depth of T mask:
	...
	function blend_color_transition(channel) {
		...
		return float_to_int_in_clamped_range(
			(float_a * colorRatioAbove)
		+	(float_b * colorRatioBelow)
		)|0;
	}
